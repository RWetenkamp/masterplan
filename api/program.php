...
<html>
    <head>
        ...
        <script>
            function submit() {
                ...
                request.open("GET","insertProgram.php?uni="+uni+"&name="+name+"&category="+category+"&degree="+degree+"&language="+language+"&ects="+ects+"&duration="+duration);
            ...
            function addToGroup() {
                ...
                let sentModules = [];
                var request = new XMLHttpRequest();
                request.open("GET","insertGroupitem.php?req="+reqid+"&module="+modules);
                request.addEventListener('load', function(event) {
                if (request.status >= 200 && request.status < 300) {
                    console.log(request.responseText);
            ...
            function addAdditional(id) {
                let process = document.getElementsByName("process_"+id)[0].value;
                console.log("process for id "+id+" : "+process);
                let email = document.getElementsByName("email_"+id)[0].value;
                console.log("email for id "+id+" : "+email);

                var request = new XMLHttpRequest();
                request.open("GET","updatePrograms.php?id="+id+"&process="+process+"&email="+email);
                request.addEventListener('load', function(event) {
                if (request.status >= 200 && request.status < 300) {
                    console.log(request.responseText);
                } else {
            ...
            function addRequ() {
                ...
                var request = new XMLHttpRequest();
                request.open("GET","insertRequirement.php?uni="+uni+"&program="+program+"&type="+type+"&req_name="+req_name+"&value="+value+"&unit="+unit+"&mandatory="+mandatory+"&orgroup="+orgroup);
                request.addEventListener('load', function(event) {
                if (request.status >= 200 && request.status < 300) {
                    console.log(request.responseText);
                } else {
    ...
    <body>
        ...
        <table>
            ...
            <?php
                $sql2 = "SELECT * FROM `programs` WHERE uni = ?";
        ...
        <table>
            ...
            <?php
                $sql3 = "SELECT * FROM `requirements` WHERE uni = ? ORDER BY program";
        ...
        <button onclick="window.location = '/program.php?uniid='+<?php echo $uni['id'];?>;">> Aktualisieren</button> &nbsp; &nbsp;
        <button onclick="window.location = '/program.php';">> Fortfahren</button>
        <br><br><br><br>
        <details>
            <summary>
                Ausfüllhinweise
